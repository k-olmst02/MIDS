...
def check_db_permissions(logs_conn, alerts_conn):
    if not os.path.exists(DB_PATH):
        if not recent_alert_exists(alerts_conn, "Database Integrity", 300):
            create_alert(
                alerts_conn,
                "Database Integrity",
                "high",
    ...
    if owner != AUTHORIZED_DB_USER:
        ...
        if not recent_alert_exists(alerts_conn, "Database Access Control", 300, owner):
            create_alert(
                alerts_conn,
    ...
    if mode != EXPECTED_DB_MODE:
        ...
        if not recent_alert_exists(alerts_conn, "Database Permission Error", 300, oct(mode)):
            create_alert(
                alerts_conn,
...
def check_honeypot_access(logs_conn, alerts_conn):
    ...
    if rows:
        ...
        if not alert_exists(alerts_conn, "Honeypot Access", newest):
            if not recent_alert_exists(alerts_conn, "Honeypot Access", 300):
                create_alert(
                    alerts_conn,
                    "Honeypot Access",
                    "critical",
    ...
    for filepath in CRITICAL_FILES:
        ...
        if FILE_HASHES[filepath] != current_hash:
            ...
            if not alert_exists(alerts_conn, "File Integrity Violation", event_ref):
                if not recent_alert_exists(alerts_conn, "File Integrity Violation", 300, filepath):
                    create_alert(
                        alerts_conn,
...
def check_privilege_escalation(logs_conn, alerts_conn):
    ...
    if rows:
        ...
        if not alert_exists(alerts_conn, "Privilege Escalation Attempt", newest):
            if not recent_alert_exists(alerts_conn, "Privilege Escalation Attempt", 300):
                create_alert(
                    alerts_conn,
...
def check_event_escalation(logs_conn, alerts_conn):
    ...
    if count >= EVENT_ESCALATION_THRESHOLD:
        if not recent_alert_exists(alerts_conn, "Event Escalation", 300):
            create_alert(
                alerts_conn,
                "Event Escalation",
                "critical",
...
def check_suspicious_ports(logs_conn, alerts_conn):
    ...
    if rows:
        ...
        if not alert_exists(alerts_conn, "Suspicious Port Activity", newest):
            if not recent_alert_exists(alerts_conn, "Suspicious Port Activity", 300):
                create_alert(
                    alerts_conn,
                    "Suspicious Port Activity",
                    "high",
