...
def check_suspicious_ports(logs_conn, alerts_conn):
    ...
    # Build a query to check for messages containing suspicious ports
    port_conditions = " OR ".join([f"LOWER(message) LIKE '%{port}%'" for port in SUSPICIOUS_PORTS])
        ...
        if not alert_exists(alerts_conn, "Suspicious Port Activity", newest):
            if not recent_alert_exists(alerts_conn, "Suspicious Port Activity", 30):
                create_alert(
                    ...
                    "Suspicious Port Activity",
                    "high",
                    f"Suspicious port activity detected. {len(rows)} event(s)",
                    newest,
                )
# --------------------------
# Main Loop
